...
import numpy as np
from loader_dino_kfold_1 import TeethDataset, split_data   # 수정
from loader_dino_kfold_1 import transforms     # 수정
...
train_files, train_labels, _, _, test_files, test_labels = split_data(parent_dir, categories, split_ratios)
combined_files = train_files
combined_labels = np.array(train_labels)

num_folds = 5
mskf = MultilabelStratifiedKFold(n_splits=num_folds, shuffle=True, random_state=42)

for fold, (train_idx, val_idx) in enumerate(mskf.split(combined_files, combined_labels)):
    print(f"Starting fold {fold+1}/{num_folds}")
    
    train_fold_files = [combined_files[i] for i in train_idx]
    train_fold_labels = combined_labels[train_idx]
    val_fold_files = [combined_files[i] for i in val_idx]
    val_fold_labels = combined_labels[val_idx]
    
    train_dataset_fold = TeethDataset(train_fold_files, train_fold_labels, transform, augment=True)
    val_dataset_fold = TeethDataset(val_fold_files, val_fold_labels, transform, augment=False)
...
model_save_directory = '/home/gpu/Workspace/youmin/Teeth_Image_Segmentation/LBA/disease_detection_MultiLabelCL/saved_dinov2_kfold_0401'
...
for epoch in range(30):
    ...
    val_loss, val_acc = evaluate_model(model, val_loader_fold, criterion, device, confmat)
    val_confmat = MultilabelConfusionMatrix(num_labels=9)
    print("Validation Confusion Matrix:")
    # print_confusion_matrix(val_confmat)
    print(f"Epoch {epoch+1} - Fold {fold+1}: Train Loss: {running_loss / total:.4f}, Val Loss: {val_loss:.4f}, Val Accuracy: {val_acc:.4f}")
