...
class TeethDataset(Dataset):
    def __init__(self, file_paths, labels, transform=None, augment=False):
        ...
        self.class_counts = self._calculate_class_counts(labels)
        self.max_count = max(self.class_counts.values())  # 가장 많은 클래스의 샘플 개수 -> train 기준 K01의 641개
        print(f"max_count: {self.max_count}")
        self.max_count = 600
    ...
    def __getitem__(self, idx):
        ...
        if self.augment:
            ...
            augment_times = self.max_count // self.class_counts[class_id] - 1
            if augment_times > 0 and random.random() < (augment_times / (augment_times + 1)):
    ...
    def _calculate_class_counts(self, labels):
        counts = np.sum(labels, axis=0)
        return {i: int(count) for i, count in enumerate(counts)}
