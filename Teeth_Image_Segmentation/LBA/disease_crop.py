...
def find_decay_teeth(json_path, image_root, output_folder, teeth_category_symbols, margin):
    ...
    image_ids_category_41 = [ann['image_id'] for ann in data['annotations'] if ann['category_id'] == 41]
    print("Image_ids with category_id 41:", image_ids_category_41)
    print(len(image_ids_category_41))

    for image_id in image_ids_category_41:
        bbox_category_41_all = [ann['bbox'] for ann in data['annotations'] if ann['image_id'] == image_id and ann['category_id'] == 41]
        print("Image ID:", image_id, "\nBbox for category_id = 41:", bbox_category_41_all)
        
        for bbox_category_41 in bbox_category_41_all:
            print("Bbox for category_id = 41:", bbox_category_41)
            ...
            for tooth_category_id in teeth_category_ids:
                ...
                for bbox_tooth in bbox_teeth:
                    ...
                    if is_bbox_contained(bbox_tooth, bbox_category_41):
                        image_info = next(item for item in data['images'] if item['id'] == image_id)

                        image_path = os.path.join(image_root, image_info['file_name'])
                        output_path = os.path.join(output_folder, f"{image_id}_{tooth_category_id}_decay_teeth_{bbox_tooth[0]}_{bbox_tooth[1]}.png")

                        print("Cropping and saving:", output_path)

                        crop_and_save(image_path, output_path, bbox_tooth, margin)
...
if __name__ == "__main__":
    ...
    output_folder = "/home/gpu/Workspace/youmin/Teeth_Image_Segmentation/LBA/cropped_images/cropped_K09_images"
    teeth_category_symbols = ['#', '@', 'teeth3']  
    margin = 30

    find_decay_teeth(json_path, root, output_folder, teeth_category_symbols, margin)

cropped_image_path = '/home/gpu/Workspace/youmin/Teeth_Image_Segmentation/LBA/cropped_images/cropped_K09_images'
