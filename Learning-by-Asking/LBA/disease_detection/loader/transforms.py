    ...
    def forward(
        ...
        if torch.rand(1) < self.p:
            ...
            if target is not None:
                ...
                if "pseudo_masks" in target:
                    target["pseudo_masks"] = target["pseudo_masks"].flip(-1)
...
class FixedSizeCrop(nn.Module):
    ...
    def _pad(self, img, target, padding):
        ...
        if target is not None:
            ...
            if "pseudo_masks" in target:
                target["pseudo_masks"] = F.pad(target["pseudo_masks"], padding, 0, "constant")
    ...
    def _crop(self, img, target, top, left, height, width):
        ...
        if target is not None:
            ...
            if "pseudo_masks" in target:
                target["pseudo_masks"] = F.crop(target["pseudo_masks"][is_valid], top, left, height, width)
...
class RandomShortestSize(nn.Module):
    ...
    def forward(
        ...
        if target is not None:
            ...
            if "pseudo_masks" in target:
                target["pseudo_masks"] = F.resize(
                    target["pseudo_masks"], [new_height, new_width], interpolation=InterpolationMode.NEAREST
                )
...
class Resize(nn.Module):
    ...
    def forward(
        ...
        if target is not None:
            ...
            if "sp_masks" in target:
                # new_height, new_width = 32, 32
                target["sp_masks"] = F.resize(
                    target["sp_masks"], [new_height, new_width], interpolation=InterpolationMode.NEAREST
                )
