...
from loader import loader_dino_4class
...
categories = ['cropped_K01_images', 'cropped_K02_images', 'cropped_K05_images', 'cropped_K09_images'] # 5개의 카테고리

split_ratios = {'train': 0.7, 'val': 0.15, 'test': 0.15}

train_files, train_labels, val_files, val_labels, test_files, test_labels = loader_dino_4class.split_data(parent_dir, categories, split_ratios)

train_dataset = loader_dino_4class.TeethDataset(train_files, train_labels, transform, augment=True)
val_dataset = loader_dino_4class.TeethDataset(val_files, val_labels, transform, augment=False)
test_dataset = loader_dino_4class.TeethDataset(test_files, test_labels, transform, augment=False)

train_loader = DataLoader(train_dataset, batch_size=32, shuffle=True)
val_loader = DataLoader(val_dataset, batch_size=32, shuffle=False)
test_loader = DataLoader(test_dataset, batch_size=32, shuffle=False)

model = model_dino_vit14b.CustomDINOV2(num_classes=4).to(device)  



criterion = nn.BCEWithLogitsLoss()
optimizer = Adam(model.parameters(), lr=0.000005)
confmat = MultilabelConfusionMatrix(num_labels=4) # 수정 필요
...
model_save_directory = '/home/gpu/Workspace/youmin/Learning-by-Asking/LBA/disease_multilabelclassification/checkpoints/saved_dinovit14b_4class_0416'
...
for epoch in range(30):
    ...
    val_confmat = MultilabelConfusionMatrix(num_labels=4)
...
test_confmat = MultilabelConfusionMatrix(num_labels=4)
